...
{
  // 1. Configure Global Precision per Proposal Section 2.1
...
// --- MICE FLOWS SETUP ---
  ...
  for (uint32_t i = 0; i < nSenders; ++i) {
      ...
      sourceApps.Start (Seconds (1.0 + (i * 0.001))); // Staggered start 
      sourceApps.Stop (Seconds (10.0));
  }

  // 4. File Output Setup
  AsciiTraceHelper ascii;
  Ptr<OutputStreamWrapper> cwndStream = ascii.CreateFileStream ("swift-incast-mice-cwnd.data");
  Ptr<OutputStreamWrapper> rttStream = ascii.CreateFileStream ("swift-incast-mice-rtt.data");
  Ptr<OutputStreamWrapper> qStream = ascii.CreateFileStream ("swift-incast-mice-queue.data");
  ...
  monitor->SerializeToXmlFile("swift-incast-mice-flowmon.xml", true, true);
  Simulator::Destroy ();
