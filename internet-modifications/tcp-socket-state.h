...
public:
  ...
  // --- CUSTOM SWIFT PROTOCOL VARIABLES ---  changes made
  uint32_t retransmit_count = 0;
  Time t_last_decrease = Seconds (0.0);
  bool m_canDecrease = true;
  double m_gamma = 0.5;
  // ---------------------------------------
  ...
  //--------------------------------------------------------------------------------
  // Add this near the other variables like m_cWnd and m_ssThresh
  uint8_t m_rcvTtl; //!< The Time-To-Live (TTL) extracted from the latest IP packet
  //-----------------------------------------------------------------------------------------
